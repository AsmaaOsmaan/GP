...
class DB{
  Future getDocs(String ID) async {
    ...
    for (int i = 0; i < querySnapshot.documents.length; i++) {
      ...
      await Firestore.instance.collection("groups").where('groupID', isEqualTo: a.documentID).getDocuments().then((snap){
...
*/
  ...
  Future getDocPages(String ID) async {
    List<QuerySnapshot> queries =[];
    ...
    for (int i = 0; i < querySnapshot.documents.length; i++) {
      ...
      await Firestore.instance.collection("pages").where('pageID', isEqualTo: a.documentID).getDocuments().then((snap){
        print('snap.runtimeType ${snap.runtimeType}');

        queries.add(snap);
      });
    }
    print("queries.lengthtt ${queries.length}");
    return queries;
...
//  Future<List<QuerySnapshot>> getSuggestions(String suggestion){
//    return
//
//         Firestore.instance.collection("groups").where('groupID', isEqualTo: suggestion).getDocuments().then((snap){
