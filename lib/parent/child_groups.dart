...
class ChildGroups extends StatefulWidget {
  final    ID;
  const ChildGroups({Key key, this.ID}) : super(key: key);
  @override
            ...
            for(var item in items){
              ...
              for(int i = 0 ;i < item.documents.length  ;i++){
                print('ddddddddddddddddddddddddddddddddddd');
                  ...
                  name.add(item.documents[i]['groupName']);
                photo.add(item.documents[i]['cover_photo']);
                GroupIDs.add(item.documents[i]['groupID']);
            ...
            return ListView.builder(
                ...
                itemBuilder:(context, index){
                 // return Center(child: Text(name[index]),);
                  return  BodyOfScreen(document:name[index],documment:photo[index],docummment:GroupIDs[index],ID:widget.ID);
                } );
...
class _BodyOfScreenState extends State<BodyOfScreen> {
  ...
  @override
  Widget build(BuildContext context) {
   Firestore.instance.collection('User').document(widget.ID).collection('groups').document(widget.docummment).get().then((DocumentSnapshot result) async{
    ...
    return    Card(
      child: InkWell(
        onTap: (){
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => GroupScreen(widget.docummment)),
          );
        },
        splashColor: Colors.yellow,
        highlightColor: Colors.blue.withOpacity(0.5),
        ...
        child: Padding(
            ...
            child: Row(
              ...
              IconButton(icon: Icon(Icons.delete_forever,),onPressed: (){
    Firestore.instance.collection('User').document(widget.ID).collection('groups').document(widget.docummment).delete();
    Firestore.instance.collection('groups').document(widget.docummment).collection('members').document(widget.ID).delete();
                ...
                Switch(value:switchval,onChanged:(bool val){
                  ...
                  Firestore.instance.collection('groups').document(widget.docummment).collection('members').document(widget.ID).updateData({'statues':switchval});
                  Firestore.instance.collection('User').document(widget.ID).collection('groups').document(widget.docummment).updateData({'statues':switchval});
